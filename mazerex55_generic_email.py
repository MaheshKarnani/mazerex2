    ...
    if not timeout_flag:
        ...
        if tube_active and start_time+interval<datetime.now():
            if beam1_detect.value==1:#unobstructed
                ...
                start_time=datetime.now()
        if state3>1000 or state4>1000 and not email_alert_sent: #error detection
            send_email=True
        if send_email and not email_alert_sent:
            print("mailing user about error mode")
            try:
                ...
                # send email
                yag.send(
                    to="mkarnani@ed.ac.uk",
                    subject="Rex2 error",
                    contents="""
                    lindzey tube has taken 1000 consecutive reads
                    """,
                )
